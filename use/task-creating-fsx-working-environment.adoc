...
* IAM 角色的 ARN 、賦予工作負載工廠為 ONTAP 檔案系統建立 FSX 所需的權限。link:task-setting-up-permissions-fsx.html["瞭解如何授予 AWS 帳戶權限"^]。
...
.關於這項工作
當您或使用「 * 我的資產 * 」頁面時，您可以探索適用於 ONTAP 檔案系統的 FSX <<建立FSX以利ONTAP 不工作環境>>。
